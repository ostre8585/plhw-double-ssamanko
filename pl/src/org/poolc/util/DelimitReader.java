...
public class DelimitReader extends Reader {
	 ...
	 *                If sz is <= 0
	 */
	public DelimitReader(Reader in, int sz, char delimiter) {
		super(in);
		if (sz <= 0)
	 ...
	 *            A Reader
	 */
	public DelimitReader(Reader in, char delimiter) {
		this(in, defaultCharBufferSize, delimiter);
	}

	public DelimitReader(Reader in) {
	 ...
	 *                If an I/O error occurs
	 */
	String readNext(boolean ignoreLF) throws IOException {
		StringBuffer s = null;
		int startChar;
		...
		synchronized (lock) {
			...
			for (;;) {
				...
				/* Skip a leftover '\n', if necessary */
				if (omitLF && (cb[nextChar] == this.delimiter))
					nextChar++;
				skipLF = false;
				omitLF = false;
